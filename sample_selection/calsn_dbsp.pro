...
;itime in second
    ...
    if keyword_set(perang) then begin
       ...
       skyphotons = texp*f_sky*area*throughput*1.d-8 ;number of photons per ang - assume area is 1 arcsec^2
       noise = sqrt(photons+readnoise*npix_seeing*npix_dispersion+skyphotons)
       snr = photons / noise 
    endif else begin 
       photons = texp*f_lambda*dlambda*area*throughput*seeing_loss ;nphotons/pixel
       skyphotons = texp*f_sky*dlambda*area*throughput ;nphotons/pixel
       noise = sqrt(photons+readnoise+skyphotons)
       snr = photons/noise
    endelse
return,snr 
end
