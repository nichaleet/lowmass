...
hrs_per_night = 6.5
exptime_ref = 10 ;mins
SN_ref  = 34 ;at 4500 Ang ;this is for magnitude 18
...
for i=0,n_elements(itime_arr)-1 do begin
   ...
   sn = fltarr(ngals)+sn_ref*sqrt(itime/exptime_ref)
   ...
   for iiter=0,niter-1 do begin
	...
	get_additional_uncertainties_lowmass,obs_feh_enk,sn,deltaobsage_enk,deltaobsfeh_upper_enk,deltaobsfeh_lower_enk,/sdss
        ...
        if iiter eq iterplot then begin
           set_plot,'ps'
           psname='int_time'+sigfig(itime,2)+'mins_'+sigfig(n_nights,2)+'nights_example_mzr.eps'
           device,filename=psname,xsize=15,ysize=10,/encapsulated,/color
	   !p.font = 0
           !p.multi=[0,1,1]
           plot,[mass,mass],[obs_feh_nl,obs_feh_enk],xtitle='logmass',ytitle='[Fe/H]',psym=1
           oploterror,mass,obs_feh_nl,deltaobsfeh_nl,psym=1,errcolor=fsc_color('red')
   ...
   psname='int_time'+sigfig(itime,2)+'mins_'+sigfig(n_nights,2)+'nights_ks_param.eps'
...
save,outstr,filename='testslope.sav'
