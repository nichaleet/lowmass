function make_chisqarr_lris,refa,refz,sn
common degen, spec_Arr, grid_feh, grid_age
	...
	for i=0,n_elements(spec_arr)-1 do begin
		...
		mgii_reg1 = where(lambda gt 4895. and lambda lt 5301,cmgii,complement=goodreg) 
		chisqarr(iz,ia) = total((refspec(goodreg)-spec_arr[i].spec(goodreg))^2/refspec_err(goodreg))				
	...
	dir='lrisoutputsn'+strtrim(string(fix(sn)),2)
	if file_test(dir,/directory) eq 0 then file_mkdir,dir
	writefits,dir+'/chisq_age'+sigfig(ageref,2)+'_feh'+sigfig(fehref,4)+'.fits',chisqarr,header
