...
pro sps_fit::fit, science, noredraw=noredraw, nostatusbar=nostatusbar
   ...
   ;;make the initial guesses unfix but within limits except redshift
    ...
    openw,copynum,'/scr2/nichal/workspace_lowmass/sps_fit/logsps/sps_fit_sdss'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
...
;    endif else pi[2].limits = [40.,400.]; [30.,200.]
   ...
   xmp = reallambda[won]
    ...
    openw,long(copynum),'/scr2/nichal/workspace_lowmass/sps_fit/logsps/sps_fit_sdssalpha'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum),systime()
    printf,long(copynum), strtrim(science.objname, 2)+'  ('+strtrim(string(self.i+1, format='(I3)'), 2)+' / '+strtrim(string(self.nspec, format='(I3)'), 2)+')'
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
...
pro sps_fit::fitomg, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    pi[0].limits = [-1.,-0.3] ;this is just for initial parameters
    pi[1].limits = [min(spsage),(galage(znow,1000)/1.e9)<max(spsage)]
   ...
   ; pi[0].limits[1]=0.4
    ...
    openw,long(copynum),'/scr2/nichal/workspace_lowmass/sps_fit/logsps/sps_fit_cl0024'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum),systime()
    printf,long(copynum), strtrim(science.objname, 2)+'  ('+strtrim(string(self.i+1, format='(I3)'), 2)+' / '+strtrim(string(self.nspec, format='(I3)'), 2)+')'
...
;    endif else pi[2].limits = [40.,400.]; [30.,200.]
   ...
   ;;make the initial guesses unfix but within limits except redshift
    ...
    openw,long(copynum),'/scr2/nichal/workspace_lowmass/sps_fit/logsps/sps_fit_glorious'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum),systime()
    printf,long(copynum), strtrim(science.objname, 2)+'  ('+strtrim(string(self.i+1, format='(I3)'), 2)+' / '+strtrim(string(self.nspec, format='(I3)'), 2)+')'
    printf,long(copynum), 'prior range:',pi.limits
...
pro sps_fit::cal_uncertainties_alpha, science
   ...
   npix = n_elements(refspecstr.lambda)
   refspec_err = abs(refspecstr.spec/science.sn)
   refspec = refspecstr.spec+randomn(seed,npix)*refspec_err
   ;make chisqarr
   gridsize = size(grid_feh,/dimensions)
   chisqarr = fltarr(gridsize)
...
pro sps_fit::continuum, science
    ...
    endcase
    contdiv = science.spec/cont
    contdivivar = science.ivar*cont^2.
    ;smooth
    if usesmooth then begin
       vsmooth = 200.
       clight = 299792.458
       contdiv_smooth = smooth_gauss_wrapper(lambda,contdiv, lambda, vsmooth/clight*lambda,ivar1=contdivivar,ivar2=contdivivar_smooth)
       contdiv = contdiv_smooth
       contdivivar = contdivivar_smooth
       science.smoothvel = vsmooth
    endif else science.smoothvel = 0.
...
pro sps_fit::statusbox, science=science
    ...
    widget_control, widget_info(self.base, find_by_uname='curmstar'), set_value=science.logm50 gt 0 ? strcompress(string(science.logmstar, format='(D10.2)'), /rem)+','+strcompress(string(science.logm50, format='(D10.2)'), /rem) : unknown
...
pro sps_fit::getscience, files=files
    ...
    if ~file_test(sciencefits) then begin
        ...
        cat = mrdfits('/scr2/nichal/workspace_lowmass/proposal_cwi/combine_targets.fits',1)
        c = n_elements(files)
        plates = strarr(c)
        MJDs = strarr(c)
        fibers = strarr(c)
           ...
           ;fix number of elements to match npix
            ...
            ;getting mass from Gallazzi's catalog
             wmatch = where(cat.plate eq science.plate and cat.mjd eq science.mjd and $
                            cat.fiberid eq science.fiber,cmatch)
             if cmatch eq 1 then begin
                 science.logm16 = cat[wmatch].logmass_min
                 science.logm50 = cat[wmatch].logmass
                 science.logm84 = cat[wmatch].logmass_max

                 science.logmstar = cat[wmatch].logmass
                 science.petromag_u = cat[wmatch].petromag_u
                 science.petromag_g = cat[wmatch].petromag_g
		 science.petromag_r = cat[wmatch].petromag_r
		 science.petromag_i = cat[wmatch].petromag_i
		 science.petromag_z = cat[wmatch].petromag_z
		 science.petromagerr_u = cat[wmatch].petromagerr_u
		 science.petromagerr_g = cat[wmatch].petromagerr_g
		 science.petromagerr_r = cat[wmatch].petromagerr_r
		 science.petromagerr_i = cat[wmatch].petromagerr_i
		 science.petromagerr_z = cat[wmatch].petromagerr_z
		 science.extinction_u = cat[wmatch].extinction_u
		 science.extinction_g = cat[wmatch].extinction_g
		 science.extinction_r = cat[wmatch].extinction_r
		 science.extinction_i = cat[wmatch].extinction_i
		 science.extinction_z = cat[wmatch].extinction_z
             endif else stop,'cannot find match'
        ...
        self->writescience
        self->specres_mask, self.directory
        speclist = strtrim(string(scienceall.plate), 2)+' '+strtrim(string(scienceall.mjd), 2)+' '+strtrim(string(scienceall.fiber), 2)
        widget_control, widget_info(self.base, find_by_uname='filelist'), set_value=speclist
...
pro sps_fit::initialize_directory, directory=directory
    common mask_in, mask_in, copynum
    newdirectory = '/scr2/nichal/workspace_lowmass/sps_fit/data/'+mask_in+'/'
    ...
    files = cat.specfile
    good = where(file_test(files) eq 1, c)
    files = files(good)
...
pro sps_fit_sdss_lowmass,mask=mask,copyi=copyi
    common mask_in, mask_in, copynum
    if ~keyword_set(mask) then mask = 'sdss_lowmass'
    mask_in = mask
    if ~keyword_set(copyi) then copyi=1
    copynum = strtrim(string(copyi,format='(I02)'),2)
    directory = '/scr2/nichal/workspace_lowmass/casjobs/spec_lowmass_allsn'
